...
def create_contact_message(message: models.ContactMessageCreate):
    values = (message.name, message.reason, message.email, message.message, message.terms_accepted)
...
def get_contact_messages(reason: str = None):
    if reason:
        result = database.execute_sql_query(queries.query_get_messages_by_reason, (reason,))
    else:
